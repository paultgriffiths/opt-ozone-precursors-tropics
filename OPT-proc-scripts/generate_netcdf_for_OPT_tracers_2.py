...
jobids=['u-cb159', 'u-cq029', 'u-cq041','u-cq330','u-ct478']
filenames=['BASE', 'GLOBAL_ANTHRO', 'GLOBAL_BB', 'TROPICAL_ANTHRO_FIX', 'EXTRATROPICAL_ANTHRO_FIX']
jobids=['u-ct478']
filenames=['EXTRATROPICAL_ANTHRO_FIX']
for jobid, filename in zip(jobids,filenames):
  ...
  for z in range(0,len(stash_list)):
    for decade in np.arange(199,202):
     ...
     outfile=outputdir+filename+'_'+decade+'0'+name[z]+'.nc'
     print(outfile)
